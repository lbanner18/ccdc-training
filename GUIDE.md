...
3. **Target-aware, with stated limits.** The baseline/recon/detection helpers
   avoid GNU-only assumptions where practical and warn rather than failing
   silently when a dependency is missing. The full mutating workflow is
   currently proven on Ubuntu/Debian with systemd: `harden.sh`'s package purge
   and offline undo use `apt-get` and cached `.deb` files, while Alpine/OpenRC,
   firewalld/SELinux, and RPM/APK package-removal paths remain future work.
   Do not treat a successful syntax check as portability proof.
...
| `baseline.sh` | `--bless`/`--approve`/`--allow`/`--remove-key` with `--apply` | The bounded question. The other detectors ask what a file CONTAINS, which is unbounded — there are infinite ways to spell a reverse shell, so a check written that way only catches the spellings someone imagined. This asks whether anything EXPLAINS a thing: in the blessed baseline, **or** package-owned with an intact checksum, **or** allowlisted. Everything else is reported, which catches mechanisms nobody enumerated in advance — two drill footholds walked past every content check in the kit and this found both. `--bless` freezes a known-good state (including the sudoers ruleset *and* membership of the groups that grant root, because `usermod -aG sudo` needs no sudoers file), and drift is measured against that forever, never against the previous pass. Every finding carries what approving it will do, the command that does it, and `--explain N` for which of the three tests failed and why. |
...
| Fast non-root regression suite | `redteam/self-test.sh`, **391 assertions across its active sub-suites, 0 failures**. The newest is `pasteable-self-test.sh`, which checks what the tools PRINT rather than what they detect: no command that cannot be pasted, no remediation that damages the operator's own box, no dry run that claims past-tense success, no flag a tool accepts without documenting. It exists because a human driving the kit for one afternoon found roughly thirty-five defects that 199 detection assertions had not. |
