...
ccdc_record_shell "$evidence/ssh.txt" 'for f in /etc/ssh/sshd_config /etc/ssh/sshd_config.d/*; do [ -r "$f" ] && { echo "--- $f ---"; sed -n "1,260p" "$f"; }; done; printf "\n--- authorized key metadata ---\n"; find /root /home -type f -name authorized_keys -readable -exec ls -l {} \; 2>/dev/null || true; find /root /home -type f -name authorized_keys -readable -exec sha256sum {} \; 2>/dev/null || true'
