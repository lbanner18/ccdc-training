...
ccdc_record_shell "$evidence/ssh.txt" 'for f in /etc/ssh/sshd_config /etc/ssh/sshd_config.d/*; do [ -r "$f" ] && { echo "--- $f ---"; sed -n "1,260p" "$f"; }; done; printf "\n--- authorized key metadata ---\n"; find /root /home -type f -name authorized_keys -readable -exec ls -l {} \; 2>/dev/null || true; find /root /home -type f -name authorized_keys -readable -exec sha256sum {} \; 2>/dev/null || true; printf "\n--- authorized_keys contents (an unknown key here survives a password change) ---\n"; find /root /home -type f -name authorized_keys -readable 2>/dev/null | while IFS= read -r kf; do echo "=== $kf ==="; sed -n "1,80p" "$kf"; done'
ccdc_record_shell "$evidence/scheduled-tasks.txt" 'printf "%s\n" "--- cron directories ---"; for d in /etc/cron.d /etc/cron.daily /etc/cron.hourly /etc/cron.weekly /etc/cron.monthly; do [ -d "$d" ] && find "$d" -maxdepth 1 -type f -ls; done; printf "%s\n" "--- user crontabs ---"; for u in $(awk -F: '\''$3 >= 0 {print $1}'\'' /etc/passwd 2>/dev/null); do crontab -u "$u" -l 2>/dev/null | sed "s#^#[$u] #"; done; printf "%s\n" "--- systemd timers ---"; systemctl list-timers --all --no-pager 2>/dev/null || true; printf "%s\n" "--- at queue ---"; atq 2>/dev/null || true'
...
ccdc_record_shell "$evidence/suid-capabilities.txt" 'printf "%s\n" "--- SUID/SGID files ---"; for root in /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin /opt /home /root; do [ -d "$root" ] && find "$root" -xdev -type f \( -perm -4000 -o -perm -2000 \) -ls 2>/dev/null; done; printf "%s\n" "--- file capabilities ---"; if command -v getcap >/dev/null 2>&1; then for root in /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin /opt /home /root; do [ -d "$root" ] && getcap -r "$root" 2>/dev/null; done; fi'
